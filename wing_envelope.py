...
def wing_envelope(ws: WingState):
    ...
    """
    rot1X = RotationMatrix(ws.shoulder_x,'x')
    rot1Z = RotationMatrix(ws.shoulder_z,'z')
    rot1Y = RotationMatrix(ws.shoulder_y,'y')
    rot1 = np.array(rot1X).dot(np.array(rot1Z)).dot(np.array(rot1Y))
    ey1 = np.array(rot1).dot(np.array(ey))

    rot2relY = RotationMatrix(ws.elbow_y,'y')
    rot2relX = RotationMatrix(ws.elbow_x,'x')
    rot2b = np.array(rot1).dot(np.array(rot2relY)).dot(np.array(rot2relX))
    rot2a = np.array(rot1).dot(np.array(rot2relY))

    rot4relY = RotationMatrix(ws.wrist_y,'y')
    rot4relZ = RotationMatrix(ws.wrist_z,'z')
