...
from DistPoint2Line import DistPoint2Line
from DistLine2Line import DistLine2Line


def ImproveLineNew(line, chord_leadingedge, chord_trailingedge, leadingedge, trailingedge):
    n = np.size(line[0]) 
    ...
    for i in range(1,n-1):
        ...
        [_ , _, dref1] = DistPoint2Line(leadingedge[:,jref1], line[:,i], chordvec) # If it does not work DP2L[][2] LAST ELEMENT OF OUTPUT
        [_, _, dref2] = DistPoint2Line(leadingedge[:,jref2], line[:,i], chordvec)
        ...
        for j in range(2, nle):
            [_,_,dtest] = DistPoint2Line(leadingedge[:,j], line[:,i], chordvec)
            if (dtest < dref2):
                dref2 = dtest
                jref2 = j
            if (dref2 < dref1):
        ...
        [_, _, newchordle[:,i], _, _] = DistLine2Line(leadingedge[:,jref1],levec,line[:,i],chordvec) 
        ...
        [_, _, dref1] = DistPoint2Line(trailingedge[:,jref1], line[:,i], chordvec) # If it does not work DP2L[][2] LAST ELEMENT OF OUTPUT
        [_, _, dref2] = DistPoint2Line(trailingedge[:,jref2], line[:,i], chordvec)
        ...
        for j in range(2, nte):
            ...
            [_,_,dtest] = DistPoint2Line(trailingedge[:,j], line[:,i], chordvec)
            if (dtest < dref2):
                dref2 = dtest
                jref2 = j
            if (dref2 < dref1):
        ...
        [_, _, newchordte[:,i], _, _] = DistLine2Line(trailingedge[:,jref1],tevec,line[:,i],chordvec) 
